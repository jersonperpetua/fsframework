...
- (id)initWithContentRect:(NSRect)contentRect styleMask:(unsigned int)aStyle backing:(NSBackingStoreType)bufferingType defer:(BOOL)flag {
	if (self = [super initWithContentRect:contentRect styleMask:aStyle backing:bufferingType defer:flag])
	{
		...
		[[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(windowWillClose:) name:NSApplicationWillTerminateNotification object:NSApp]; // in case the user quits with the preference window open
		return self;
	} else {
		return nil;
	}
...
- (void)windowWillClose:(NSNotification *)notificaiton {
	if (autosave_name)
	{
