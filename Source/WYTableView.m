...
NSString *WYAscendingOrder = @"Ascending Order";
NSString *WYDescendingOrder = @"Descending Order";
...
- (void)saveHiddenTableColumnsIfNeeded {
	if ([self autosaveName]) {
		...
		[[NSUserDefaults standardUserDefaults] setObject:save forKey:[NSString stringWithFormat:@"WYTableView %@ Hidden Columns", [self autosaveName]]];
...
- (void)updateToSavedTableColumnsWithAutosaveName:(NSString *)name {
	...
	NSArray *cols = [[NSUserDefaults standardUserDefaults] objectForKey:[NSString stringWithFormat:@"WYTableView %@ Hidden Columns", name]];
...
@implementation NSObject (WYTableViewDelegate)

- (NSString *)tableView:(NSTableView *)tableView compareValueForRow:(int)row {
	[[NSException exceptionWithName:@"WYTableView delegate exception" reason:@"WYTableView must respond to tableView:(NSTableView *)tableView compareValueForRow:(int)row if selectStringOnKeyDown is set to TRUE." userInfo:nil] raise];
	return nil;
}
